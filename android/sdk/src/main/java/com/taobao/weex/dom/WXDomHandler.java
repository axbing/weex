...
class WXDomHandler implements Handler.Callback {
  ...
  private void handleCommandList(WXDomCommandList commandList) {
      for (int i = 0; i < commandList.commandIds.size(); ++i) {
          int cmd = commandList.commandIds.get(i);
          WXDomTask task = commandList.tasks.get(i);
          switch (cmd) {
  ...
  public boolean handleMessage(Message msg) {
    ...
    switch (what) {
      case MsgType.WX_DOM_COMMAND_LIST:
        handleCommandList((WXDomCommandList)obj);
    ...
    mWXDomManager.scheduleBatchIfDirty();
    return true;
  }
  ...
  public static class MsgType {
    ...
    public static final int WX_DOM_FORCE_LAYOUT = 0x0b;
    public static final int WX_DOM_COMMAND_LIST = 0xfe;
